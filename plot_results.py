...
def find_best_sample(X_test_pca, y_test, char_model, char_y_scaler, device):
    ...
    with torch.no_grad():
        pred_scaled = char_model(torch.FloatTensor(X_test_pca).to(device)).cpu().numpy()
    ...
    print(f"  Best sample: index {best_idx}, char RMSE = {per_sample_rmse[best_idx]:.6f}")
    return best_idx
...
def plot_lc_reconstruction(
    sample_idx,
    X_test_clean, X_test_pca,
    char_model, char_x_scaler, char_y_scaler, char_pca,
    gen_model, gen_x_scaler, gen_y_scaler, gen_pca,
    n_days, samples_per_day, noise_std,
    device, lsst_seed=42,
):
    """
    ...
    aug_pca = char_pca.transform(char_x_scaler.transform(aug_curve[np.newaxis, :]))
    with torch.no_grad():
        pred_params_scaled = char_model(
            torch.FloatTensor(aug_pca).to(device)).cpu().numpy()
    reconstructed = generate_curves(
        pred_params_scaled, gen_model, gen_x_scaler, gen_pca, device)[0]

    # --- Figure ---
    fig, ax = plt.subplots(figsize=(5, 3)) 

    ax.plot(time_axis, original, 'b-', linewidth=1.5, label='Original')
    ax.plot(desampled_times, desampled_vals, 'g.', markersize=5,
            label='Desampled')
    ax.plot(desampled_times, noisy_vals, 'k.', markersize=3,
            label='Noisy')
    ax.errorbar(desampled_times, noisy_vals, yerr=noise_std,
                fmt='none', ecolor='black', elinewidth=0.5, capsize=0,
                label='Error')
    #ax.axvline(x=0, color='r', linestyle='--', linewidth=1.0, alpha=0.6)
    ax.plot(time_axis, reconstructed, 'r--', linewidth=1.5,
            label='Reconstructed')
    #ax.axvline(x=0, color='r', linestyle='--', linewidth=1.0, alpha=0.6)
...
def compute_chargen_rmse(
    X_test_clean, X_test_aug,
    char_model, char_x_scaler, char_pca,
    gen_model, gen_x_scaler, gen_pca,
    device,
):
    ...
    with torch.no_grad():
        pred_params_scaled = char_model(
            torch.FloatTensor(X_aug_pca).to(device)).cpu().numpy()
    pred_curves = generate_curves(pred_params_scaled, gen_model, gen_x_scaler, gen_pca, device)
    return np.sqrt(np.mean((X_test_clean - pred_curves) ** 2, axis=0))


def plot_reconstruction_error(
    X_test_clean, X_test_aug, y_test_scaled,
    char_model, char_x_scaler, char_pca,
    gen_model, gen_x_scaler, gen_pca,
    n_days, samples_per_day, device,
):
    """
    ...
    pred_gen = generate_curves(y_test_scaled, gen_model, gen_x_scaler, gen_pca, device)
    ...
    fig, ax = plt.subplots(figsize=(5,3))
    ax.semilogy(time_axis, rmse_chargen_aug,   linewidth=1.2, label='Reconstruction error + Noise')
    ax.semilogy(time_axis, rmse_chargen_clean, linewidth=1.2, label='Reconstruction error')
...
def main():
    parser = argparse.ArgumentParser(description="Generate ASTRAI result plots.")
    parser.add_argument("--config", default="configs/default_split.yaml",
                        help="Path to split config YAML")
    parser.add_argument("--exp_char", required=True,
                        help="Characterizer experiment directory")
    parser.add_argument("--exp_gen", required=True,
    ...
    parser.add_argument("--lsst_seed", type=int, default=42,
    ...
    fig1 = plot_lc_reconstruction(
        ...
        gen_model, gen_x_scaler, gen_y_scaler, gen_pca,
        n_days, samples_per_day, noise_std,
        device, lsst_seed=args.lsst_seed,
    )
    ...
    X_test_aug = apply_lsst_pipeline(X_test_clean, n_days, noise_std,
