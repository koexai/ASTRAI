"""
main_split.py - Full pipeline for split training (preprocessing + characterizer + generator).
...
Usage::
    ...
    python main_split.py --config configs/default_split.yaml
"""
import argparse
import yaml
...
def main():
    parser = argparse.ArgumentParser(description="ASTRAI split training pipeline")
    ...
    run_characterizer_training(cfg, prep_dir=prep_dir, exp_dir=char_exp,
    ...
    run_generator_training(cfg, prep_dir=prep_dir, exp_dir=gen_exp,
                           config_path=args.config)
    ...
    print("=" * 50)
    print(f"\nInference command:")
    print(f"  python inference_split.py --exp_char {char_exp} --exp_gen {gen_exp}")
