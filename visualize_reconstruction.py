...
def reconstruct_all(model, X, x_scaler, y_scaler, pca, device):
    ...
    per_sample_rmse : np.ndarray
        RMSE per sample, shape (n_samples,).
    """
    ...
    model_reconstructed = x_scaler.inverse_transform(pca.inverse_transform(pred_curves_pca))
    per_sample_recon_rmse = np.sqrt(np.mean((X - model_reconstructed) ** 2, axis=1))

    return model_reconstructed, pred_params_sc, per_sample_recon_rmse


def plot_single(idx, X, y, model_reconstructed, x_scaler, pca, param_names, y_scaler, pred_params_sc, n_days, noise_std, per_sample_char_rmse, samples_per_day=4):
    """Plot the 3-panel visualization for a single sample."""
    original_curve = X[idx]
    reconstructed_curve = model_reconstructed[idx]

    # LSST augmentation
    augmented_curve = apply_lsst_pipeline(X[idx:idx+1], n_days, noise_std, samples_per_day=samples_per_day)[0]

    # PCA-only reconstruction
    original_scaled = x_scaler.transform(X[idx:idx+1])
    original_pca = pca.transform(original_scaled)
    pca_reconstructed = x_scaler.inverse_transform(pca.inverse_transform(original_pca))[0]

    # Print parameters
    pred_params = y_scaler.inverse_transform(pred_params_sc[idx:idx+1])[0]
    pred_params_original = np.expm1(pred_params)

    char_rmse_str = f"Char RMSE: {per_sample_char_rmse[idx]:.6f}" if per_sample_char_rmse is not None else ""
    ...
    all_vals = np.concatenate([original_curve, augmented_curve, pca_reconstructed, reconstructed_curve])
    ...
    axes[0].plot(time_axis, augmented_curve, label="Augmented (LSST)", alpha=0.7)
    ...
    axes[1].plot(time_axis, pca_reconstructed, label=f"PCA-only ({pca.n_components} comp.)", alpha=0.7, linestyle="--")
    axes[1].plot(time_axis, reconstructed_curve, label="Model reconstruction", alpha=0.7, linestyle="-.")
    ...
    axes[2].plot(time_axis, residual_pca, label="Residual (PCA-only)", alpha=0.7)
...
def main():
    parser = argparse.ArgumentParser(description="Visualize the full reconstruction pipeline.")
    ...
    if args.exp:
        model, x_scaler, y_scaler, pca = load_from_experiment(args.exp, cfg, device)
    else:
    ...
    model_reconstructed, pred_params_sc, per_sample_recon_rmse = reconstruct_all(
        model, X, x_scaler, y_scaler, pca, device
    )
    ...
    if args.top is not None:
        if per_sample_char_rmse is None:
            print("ERROR: --top requires ground-truth labels to compute characterization error.")
            return
        top_indices = np.argsort(per_sample_char_rmse)[:args.top]
        print(f"\nTop {args.top} samples with lowest characterization RMSE:")
        for rank, idx in enumerate(top_indices, 1):
            print(f"  #{rank}: sample {idx}, Char RMSE = {per_sample_char_rmse[idx]:.6f}")
    ...
    for idx in indices_to_plot:
        plot_single(idx, X, y, model_reconstructed, x_scaler, pca,
                    param_names, y_scaler, pred_params_sc, n_days, noise_std,
                    per_sample_char_rmse, samples_per_day=samples_per_day)
